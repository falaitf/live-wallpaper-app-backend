...
const s3 = new S3Client({
  region: process.env.AWS_BATTERY_BUCKET_REGION,
  credentials: {
...
const uploadToS3 = async (file, folder = "") => {
  ...
  const upload = new Upload({
    client: s3,
    params: {
      Bucket: process.env.AWS_BATTERY_BUCKET_NAME,
      Key: key,
      Body: file.buffer,
  ...
  return `https://${process.env.AWS_BATTERY_BUCKET_NAME}.s3.${process.env.AWS_BATTERY_BUCKET_REGION}.amazonaws.com/${key}`;
};
